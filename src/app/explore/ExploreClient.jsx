...
function TierBadge({ tier, small }) {
  ...
  return (
    <span style={{
      fontFamily:'var(--mono)', fontSize: small ? '0.62rem' : '0.68rem',
      letterSpacing:'0.08em', textTransform:'uppercase',
      padding: small ? '0.2rem 0.5rem' : '0.25rem 0.65rem',
      background: color, color:'#faf8f3', display:'inline-block', whiteSpace:'nowrap',
    }}>{tier}</span>
  );
}
...
function ScoreBar({ score }) {
  if (score === null || score === undefined)
    return <span style={{fontFamily:'var(--mono)',fontSize:'0.7rem',color:'var(--muted)'}}>N/A</span>;
  ...
  return (
    <div style={{display:'flex',alignItems:'center',gap:'0.5rem'}}>
      ...
      <span style={{fontFamily:'var(--mono)',fontSize:'0.7rem',color:'var(--muted)',minWidth:'1.5rem'}}>{score}</span>
    </div>
  );
}
...
export default function ExploreClient({ initialOrgs = [] }) {
  ...
  const loadDetail = (org) => {
    ...
    fetch(`${SUPABASE_URL}/rest/v1/calibration_criterion_scores?anchor_id=eq.${org.id}&select=criterion,score&order=criterion`, {
      headers: { apikey: ANON_KEY, Authorization: `Bearer ${ANON_KEY}` }
    })
      .then(r => r.json())
      .then(data => {
        const map = {};
        data.forEach(d => { map[d.criterion] = d.score; });
            ...
            <div style={{overflowX:'auto',marginBottom:selected?'2rem':0}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                ...
                <tbody>
                  ...
                  {filtered.length === 0 && (
                    <tr><td colSpan={6} style={{padding:'3rem',textAlign:'center',color:'var(--muted)',fontFamily:'var(--mono)',fontSize:'0.8rem'}}>
              ...
              <div style={{border:'1px solid rgba(200,168,75,0.25)',background:'rgba(244,240,232,0.02)',padding:'2rem',position:'relative'}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:'1rem',alignItems:'start',marginBottom:'1.5rem'}}>
                  <div>
                    ...
                    <div style={{fontFamily:'var(--mono)',fontSize:'0.72rem',color:'var(--muted)',letterSpacing:'0.06em'}}>{selected.category} · {selected.trajectory}</div>
                  </div>
                  <TierBadge tier={selected.composite_tier} />
                </div>

                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1px',background:'rgba(212,206,196,0.1)',marginBottom:'2rem'}}>
                  {[
                ...
                {loadingDetail ? (
                  <div style={{fontFamily:'var(--mono)',fontSize:'0.75rem',color:'var(--muted)'}}>Loading criterion scores...</div>
                ) : criterionScores[selected.id] ? (
                  <div>
                    <div style={{fontFamily:'var(--mono)',fontSize:'0.65rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'var(--gold)',marginBottom:'1rem'}}>Criterion Scores</div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem 2rem'}}>
                      {CRITERIA.map(c => (
                        <div key={c} style={{display:'grid',gridTemplateColumns:'160px 1fr',gap:'0.75rem',alignItems:'center',padding:'0.4rem 0',borderBottom:'1px solid rgba(212,206,196,0.06)'}}>
                          <div>
                            <span style={{fontFamily:'var(--mono)',fontSize:'0.62rem',color:'var(--gold)',marginRight:'0.4rem'}}>{c}</span>
                            <span style={{fontSize:'0.73rem',color:'var(--muted)'}}>{CRITERIA_NAMES[c]}</span>
                          </div>
                          <ScoreBar score={criterionScores[selected.id][c]} />
                        </div>
                      ))}
                    </div>
                  </div>
